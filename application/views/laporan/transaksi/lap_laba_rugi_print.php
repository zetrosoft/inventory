<?php
		  ...
		  foreach($temp_rec as $r)
		  {
			...
			//$harga_beli=($r->Harga_Beli==0)?(0):$r->Harga_Beli;
			$kotor	=($pajak=='ok')?(($r->Margin)):($r->Margin);
			$ppn	=($pajak=='ok')?(($kotor)*10/100):0;
			$lab	=($kotor-$opr-$ppn);
			$a->Row(array($n,tglfromSql($r->Tanggal),
				number_format(round($kotor,0),2),
				number_format((int)$opr,2),
				number_format(round($ppn,0),2),
				number_format(round($lab,0),2)
				));
			//sub tlot
			$tppn	=($tppn+$ppn);
			$harga	=($harga+(round($kotor,0)));//($r->Jual-$r->Harga_Beli));
			$hgbb	=($hgbb+$opr);
			$hargaj	=($hargaj+$lab);
		  ...
		  $a->Cell(25,8,number_format(round($tppn,0),2),1,0,'R',true);
		  $a->Cell(30,8,number_format(round($hargaj,0),2),1,1,'R',true);
