...
<input type='hidden' id='no_Agt' name='no_Agt' value=''/>
<input type='hidden' id='v_stock' value=''>
</form>
<?
if($c_penjualan__index!=''){
	...
	for($i=1;$i<=15;$i++){
        $do="<input type='checkbox' id='".$i."__do' name='".$i."__do' style='cursor:pointer'/>";
	echo tr().
	...
	<td class='kotak' width='25%' valign='top' height='70px' rowspan='2'>
		  <table id='inform' style='border-collapse:collapse' width='100%'>
		  	<tr class='header'><td colspan='2' class='kotak'>Informasi Stock</td></tr>
		  	<tr><td width='40%' class='kotak angka' id='aktife'>In line Stock</td>
		  	  	<td class='kotak' id='ist'></td>
			</tr>
		  	<tr><td width='40%' class='kotak angka' id='gdg'>&nbsp;</td>
		  	  	<td class='kotak' id='mdl'></td>
			</tr>
		  	<tr><td class='kotak angka' id='mgm'>&nbsp;</td><td class='kotak' id='xst'>
	...
	<tr><td class='kotak' valign='top' align=''-->
		...
		<tr><td colspan='2' align='center'>
		  <input type='button' id='bayar' value='Bayar (F9)'>
		  <!--input type='button' id='kredit' value='Bayar Kredit'-->
		  <input type='button' id='batal' value='Batal'>
		  </td>
		 </tr></table>
         <hr><b>Do Monitoring</b>
         <table width='100%' id='domon'>
         <thead>
         <tr class='headere'>
             <th class='kotak' width='60%'>Tujuan</th>
             <th class='kotak' width='25%'>Status</th>
             <th class='kotak' width='10%'><img src='".base_url()."asset/images/12.png' class='images' id='refreshdo'></th>
        </tr>
         <thead>
         <tbody>
         </tbody>
	 ...
	 <tr><td colspan='1' class=''>
		  <table style='border-collapse:collpse'>
			  <tr align='center'>
                ...
                <td class='kotak' width='80px'>F8<br> Cetak DO</td>
			  </tr>
		  </table>
	 </td></tr>
	</table>";
...
panel_multi_end();
panel_end();
popup_start('pembayaran','Pembayaran',500);
...
popup_end();
popup_start('DO','Delivery Order',850); 
        $zfm->frm_filename('asset/bin/zetro_beli.frm');
		$zfm->AddBarisKosong(true);
...
terbilang();
//echo $nasabah;
switch($this->session->userdata('gudang'))
{
    case 1:$lokasine='PUSAT';break;
    case 3:
    case 5:$lokasine='GUDANG';break;
...
$(document).ready(function(e) {
    ...
    $('#frm11 input#notransdo').attr({'data-source':'[<?=$nomore;?>]'});
    $('#aktife').html('<?=$lokasine;?>');
});
</script>