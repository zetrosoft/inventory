...
$(document).ready(function(e) {
	...
	$('#nm_nasabah')
        ...
        .live("keypress",function(e){
            if(e.which==13){
                ...
                _get_detailNasabah($(this).val(),'xx');
            }
        })
	...
	//event untuk input textbox di grid table#listTable
	    ...
	    //==========================line 1==========================
		  $('#frm2 #1__nm_barang')
			  ...
			  .live('focus',function(){_simpan_header();_get_detailNasabah($('#nm_nasabah').val(),'zz'); })
	...
	$('#bayar').click(function(){
        ...
        $.post('hutangpelanggan',{'nm_nasabah':$('#nm_nasabah').val()},function(result){
			//alert($.trim(result).substring(0,1))
            if($.trim(result).substring(0,1)=='-')
			{
				$('#frm3 input#dibayar')
					.val(format_number(parseFloat($.trim(result)*-1)))
					.keyup();
				$('#frm3 #nota').val('0');
			}else{
				$('#frm3 #nota').val(format_number(parseFloat($.trim(result))));
			}
        ...
        var nt=($(frm+' #nota').val().replace(',',''));
            nt=nt.replace(',','');
		//(ppn==0)?lock('#ppn'):lock('#ppn');
				$(frm+' #nitip').removeAttr('checked');
				$(frm+' #nbr').removeAttr('checked');
				$('#stat_sim').val(frm);
				$('#nama').val(prs);
	...
	$('#frm3 #nitip').live('click',function(){
		//alert($(this).is('checked'))
		if($(this).is(':checked')){
			$('#frm3 #nbr').removeAttr('checked');
			$('#frm3 #nbr').attr('disabled','disabled');
		}else{
			$('#frm3 #nbr').removeAttr('disabled');
		}
	})
	$('#frm3 #nbr').live('click',function(){
		//alert($('#frm3 #nbr').is('checked'))
		if($(this).is(':checked')){
			$('#frm3 #nitip').removeAttr('checked');
			$('#frm3 #nitip').attr('disabled','disabled');
		}else{
			$('#frm3 #nitip').removeAttr('disabled');
		}
	})
	$('#frm3 #dibayar')
		.focus().select()
		.keyup(function(){
			...
			$('#jmlbayar').val(sisa);
			if(parseFloat(sisa)<=0){$('#frm3 #nitip').attr('disabled','disabled');}else{$('#frm3 #nitip').removeAttr('disabled');}
		})
		.focusout(function(){
...
*/	})
	...
	$(document).keypress(function(e){
		if(e.keyCode==112){
			...
			return false
        }else if(e.keyCode==119){//button f8
            _show_do();
 ...
 //re print slip
  ...
  $('#frm11 input#nm_pelanggane').focusin(function(){
       // if($('#frm11 input#notransdo').val().length==10){
        //    _getNamaPelanggan($('#frm11 input#notransdo').val());
       // }
  ...
  */
  ListDOMon();
    ...
    $('#frm115 #no_mobil').change(function(){
        var notrans=$('#frm115 #notransdom').val();
        var nomobil=$('#frm115 #no_mobil').val();
        _getDOtrans(notrans,nomobil);
    })
    $('#printDOM').click(function(){_cetakDOM();})
//end document ready
})
...
//}
	...
	function _simpan_pembayaran(){
        ...
        var hutang          =$(frm+' #nota').val();
        var kembaliane=(kembalian.substr(0,1)=='-')?"-"+(to_number(kembalian.replace('-',''))):to_number(kembalian.replace('-',''))
		$.post('simpan_bayar',{
			...
			'kembalian'		:kembaliane,
			'terbilang'		:terbilang,
			'cbayar'		:jenis_bayar,
			'tanggal'		:$('#tgl_transaksi').val(),
            'hutang'        :to_number(hutang),
            'nota'          :($(frm+' #chkNota').val()=='Y')?kembaliane:to_number(hutang),
            'notadibayar'   :($(frm+' #chkNota').val()),
            'nasabah'       :$('#nm_nasabah').val(),
			'nitip'			:($(frm+' #nitip').is(':checked'))?kembaliane:'0',
			'nbarang'		:($(frm+' #nbr').is(':checked'))?to_number(dibayar):'0',
            'nitipuang'     :($(frm+' #nitip').is(':checked'))?"Y":"N",
            'nitipbarang'   :($(frm+' #nbr').is(':checked'))?"Y":"N"
		},function(result){
...
*/	var n_tran=$('#frm1 input#no_transaksi').val().substr(5,5);
		...
		function(result)
		{
            ...
            var r=$.parseJSON(result);
				...
				function(data){
						...
						$('#frm100 input#v_stock').val(parseFloat(jm.stock));
	...
	{
		var jmlJual=$('#frm2 input#'+BarisAktif+'__jml_transaksi').val();
		var jmlStock=($('#frm100 input#v_stock').val());
		if(parseFloat(jmlJual) > parseFloat(jmlStock)){
			alert('Jumlah permintaan lebih besar dari stock');
			$('#frm2 input#'+BarisAktif+'__jml_transaksi').val((jmlStock)).focus().select();
		}
		...
		$('#'+BarisAktif+'__harga_total').val(parseInt(tth))
		total_harga();
	}
    ...
    function _get_detailNasabah(id,upd)
    {
     $.post(path+'member/get_member_detail',{'id':id},function(result){
         var r=$.parseJSON(result);
         //_generate_nomor_agt();
         if(r.Nama=='-'){
             _simpan_anggota($('#nm_nasabah').val(),$('#grp_nasabah').val())
         }else{
             if(upd=='xx'){$('#grp_nasabah').val(r.NIP).select();}
             $('#maxlpg').val(r.ID_Kelamin);
             $('#lpgMember').val(r.ID_Check);
             $('#nm_nasabah').val((r.Nama!='-')?r.Nama:'');
         }
	...
	function _generate_nomor_agt(){
	$.post(path+'member/get_nomor_anggota',{'id':'id'},
		function(result){
			$('#no_Agt').val(result);
		})
    }

    function _simpan_anggota(Nama,NIP){
        $.post(path+'member/set_anggota',{
						'No_Agt'	:$('#no_Agt').val(),
						'ID_Dept'	:'1',
						'NIP'		:NIP,
						'Nama'		:Nama
    ...
    function _getDONotrans(notran){
        $.post('GetListTrans',{'notrans':notran},function(rst){
            ...
            //$('#listdo').fixedHeader({width:780,height:300})
    ...
    function _getNamaPelanggan(notran){
        $.post('GetPelangganDO',{'notrans':notran},function(data){
            ...
            $('#frm11 input#nm_pelanggane').val(r.Deskripsi);
    ...
    function chk_click(id){
              $.post('Set_ListDO',{
                ...
                'nm_pelanggan':$('#frm11 #nm_pelanggane').val(),
 ...
 /* added on 06-02-2016 */
    ...
    function chk_click2(id){
        ...
        var sopir=$('#frm115 #nm_driver').val();
        var tujuan=$('#frm115 #add_kirim').val();
        if(tujuan=='' && nmb!='DiAmbil'){
            alert('Kolom Kirim ke harus di isi dengan Tujuan kirim barang tersebut');
            $('#frm215 #id_'+id).removeAttr("checked");
        }else if(sopir==''){
            alert("Nama Sopir harus di isi");
            $('#frm215 #id_'+id).removeAttr("checked");
        }else{
            $.post('UpdateDoKirim',{
                'id':id,
    ...
    function _tujuanKirim(notran)
    {
        $.post('GetPelangganDO',{'notrans':notran},function(data){
                var r=$.parseJSON($.trim(data));
            $('#frm115 #add_kirim').val(r.Deskripsi);
        });
    }
var xpath=$('#server').val();
