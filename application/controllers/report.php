...
class Report extends CI_Controller
{
	...
	function mutasi(){
		$this->zetro_auth->menu_id(array('listmutasistock'));
		$this->list_data($this->zetro_auth->auth());
	...
	function lap_penjualan(){
        ...
        $period=empty($_POST['sampai_tgl'])?
			   "where doc_date='".tglToSql($_POST['dari_tgl'])."'":
			   "where doc_date between '".tglToSql($_POST['dari_tgl'])."' and '".tglToSql($_POST['sampai_tgl'])."'";
		...
		$where.=($this->input->post('id_lok')=='')?'':" and p.ID_Close='".$this->input->post('id_lok')."'";
		$group="group by concat(dt.ID_Barang,dt.ID_Jenis,dt.Harga)";
		...
		$data['orderby']=$ordby;
        $data['idlokasi']=$this->input->post('id_lok');
	...
	function lap_penjualan_detail(){
		...
		$group="group by p.Tanggal,p.Deskripsi";
		$ordby="order by ".$this->input->post('orderby');
		$ordby.=($this->input->post('urutan')=='')?'':" ".$this->input->post('urutan');
		...
		$data['where']=$where;
		$data['orderby']=$ordby;
        $data['idlokasi']=$this->input->post('id_lokasi');
		$data['detail']=$this->input->post('show_de');
	...
	function penjualan_per_konsumen(){
		...
		$where=empty($_POST['sampai_tgl'])?
			   "where p.Tanggal='".tglToSql($_POST['dari_tgl'])."'":
			   "where p.Tanggal between '".tglToSql($_POST['dari_tgl'])."' and '".tglToSql($_POST['sampai_tgl'])."'";
		$where.=" and p.Deskripsi='".$this->input->post('ID_Anggota')."' and Jumlah!='0'";
		...
		$data['id_jenis']=rdb('inv_pembelian_jenis','Jenis_Beli','Jenis_Beli',"Where ID='".$this->input->post('jenis_beli')."'");
		//($this->input->post('show_de')=='')?
		$data['lokasi']=($this->input->post('id_lokasi')=='')?'':rdb('user_lokasi','Lokasi','Lokasi',"where ID='".$this->input->post('id_lokasi')."'");
		$data['where']=$where;
		$data['orderby']=$orderby;
